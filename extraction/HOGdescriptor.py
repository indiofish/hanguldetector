...
path_dir_database = "./hangul-images/*" # Total Dataset : hangul-images
path_dir_testdata = "./testset/*" # hangul-images 에서 가져온 몇 개 이미지들

input_filenames = glob.glob(path_dir_testdata)
filenames = glob.glob(path_dir_database)

# print(filenames)
# print(input_filenames)


name = input_filenames[25].split('\\')[1][0] # hangul-images 에서 아무거나 하나 추출
print("Input_text is {}".format(name)) # 추출한 문자 출력 ex) '가'

input_feature = HOG(input_filenames[25]) # 추출한 문자의 HoG 값

filename_selected = [file for file in filenames if file.endswith("{}.jpeg".format(name))] # 추출한 문자 ex)'가' 와 동일한 글자 이미지 파일만 리스트에 저장 (165개가 되어야 정상)
print(filename_selected)


HoG_features_book = dict()

# '가' 165개 폰트에 대한, '폰트고유넘버-HOG 값' 딕셔너리에 저장
for file in filename_selected:
    HoG_features_book[file.split('_')[1]] = HOG("{}".format(file))

print(len(HoG_features_book)) # 165개가 되어야함


# 폰트 고유넘버 출력
for font, feature in HoG_features_book.items():
    ...
    #         err_temp = feature[i] - input_feature[i]
    #         error[error.keys[i]] = np.square(err_temp)
